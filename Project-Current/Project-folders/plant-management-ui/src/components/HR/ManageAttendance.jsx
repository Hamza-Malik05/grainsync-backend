...
export default function ManageAttendance() {
  ...
  const handleClockIn = async (attendanceId) => {
    setLoading(true);
    try {
      await axios.put(`http://localhost:8080/api/attendance/${attendanceId}/clock-in`);
  ...
  const handleClockOut = async (attendanceId) => {
    setLoading(true);
    try {
      await axios.put(`http://localhost:8080/api/attendance/${attendanceId}/clock-out`);
      await fetchAttendanceRecords();
    } catch (error) {
  ...
  const handleMarkAbsent = async (attendanceId) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:8080/api/attendance/${attendanceId}/mark-absent`);
